...
아키텍처를 하나씩 조립해 나가는 과정에서 직면한 핵심 문제들과 이를 해결하기 위한 주요 의사결정입니다.

특히 가장 까다로웠던 부분은 '분산된 비동기 환경에서 다중 파일의 상태를 동기화하고 예외를 안전하게 처리하는 것'이었습니다.

초기 설계에서는 사용자가 파일 업로드를 시도하는 즉시(Presigned URL 발급 시점) 백엔드에서 Job(전체 업로드 단위)과 File 엔티티를 선제적으로 생성하고, 이후 S3 업로드와 AI 라벨링 로직이 순차적으로 진행되도록 구상했습니다. 하지만 이 구조는 클라이언트 측의 네트워크 문제나 변심으로 인해 실제 S3 업로드가 실패(또는 취소)할 경우, DB에 실체가 없는 '좀비 데이터'가 무분별하게 남게 되며 예외 처리와 롤백의 책임을 백엔드가 감당하기 매우 복잡해진다는 치명적인 문제가 있었습니다.

이러한 트랜잭션 관리의 한계와 비동기 파이프라인의 불확실성을 완벽히 통제하기 위해 다음과 같은 구체적인 의사결정들을 내렸습니다.
